...
# MAGIC %md This notebook was developed to run on a minimal (one-worker node) **Databricks 8.3 cluster**. You should leave this notebook running on this cluster while you run the remaining notebooks on a separate cluster.

# COMMAND ----------

# MAGIC %md The purpose of this notebook is to generate a stream of inventory-relevant data originating from two simulated stores, one physical and the other online.  These data are transmitted to various data ingest services configured with the cloud provider as indicated in the *POS 01: Environment Setup notebook*.
# MAGIC 
...
! mkdir -p /dbfs/mnt/pos/generator /dbfs/mnt/pos/static_data /dbfs/mnt/pos/inventory_snapshots /dbfs/mnt/pos/checkpoints/inventory_change /dbfs/mnt/pos/checkpoints/inventory_snapshot /dbfs/mnt/pos/checkpoints/inventory_current

# COMMAND ----------

!mv /dbfs/mnt/pos/inventory_change_online.txt /dbfs/mnt/pos/generator/.
!mv /dbfs/mnt/pos/inventory_snapshot_store001.txt /dbfs/mnt/pos/generator/.
!mv /dbfs/mnt/pos/inventory_snapshot_online.txt /dbfs/mnt/pos/generator/.
!mv /dbfs/mnt/pos/store.txt /dbfs/mnt/pos/static_data/store.txt
!mv /dbfs/mnt/pos/item.txt /dbfs/mnt/pos/static_data/item.txt
!mv /dbfs/mnt/pos/inventory_change_type.txt /dbfs/mnt/pos/static_data/inventory_change_type.txt

# COMMAND ----------

! mv /dbfs/mnt/pos/inventory_change_store001.txt /dbfs/mnt/pos/generator/.
...
display(inventory_change1.collect())
...
! cat  /dbfs/mnt/pos/static_data/inventory_change_type.txt
...
for event in inventory_change:
  ...
  if len(event) > max_msg_size:
    ...
    for i, item in enumerate(items): # for each item
      d['items'] = item   # add a one-item items-collection
